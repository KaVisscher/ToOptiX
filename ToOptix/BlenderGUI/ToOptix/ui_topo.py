...
class OBJECT_OT_start_topo_optimization(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        iterations = scene.topoOpt.n_const
        ...
        weightStatic = scene.topoOpt.weight_static

        thermIsActive = scene.topoOpt.heat_trans_topo
        heatPath = scene.topoOpt.heat_trans_path
        weightTherm = scene.topoOpt.weight_heat_transfer




        TopologyConfig.run_optimization(volfrac, penal, workDir, solverPath, matSets, iterations, StructIsActive, staticPath, weightStatic, thermIsActive, heatPath, weightTherm)
